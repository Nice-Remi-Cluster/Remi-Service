...
class UserBind(models.Model):
    ...
    bind_name = fields.CharField(max_length=30)
    ...
    async def create_bind(cls, user: User, bind_type: UserBindType, bind_content: str,
        ...
        创建一个新的用户绑定
        """
        related_bind_count = await cls.filter(user=user, bind_type=bind_type).count()

        bind = cls(
            user=user,
            bind_type=bind_type,
            bind_content=bind_content,
            bind_name=bind_name or f"{related_bind_count}",
    ...
    @classmethod
    async def get_user_binds(cls, user: User, bind_type: UserBindType) -> list["UserBind"]:
        """
        获取用户指定类型的所有绑定
        """
        return await cls.filter(user=user, bind_type=bind_type)
